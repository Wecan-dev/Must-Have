...
<tr class="refund <?php echo ( ! empty( $class ) ) ? $class : ''; ?>" data-order_refund_id="<?php echo dokan_get_prop( $refund, 'id' ); ?>">
	...
	<td class="name">
		<?php
			echo esc_attr__( 'Reembolso', 'dokan' ) . ' #' . absint( dokan_get_prop( $refund, 'id' ) ) . ' - ' . esc_attr( date_i18n( get_option( 'date_format' ) . ', ' . get_option( 'time_format' ), strtotime( $refund_post->post_date ) ) );
