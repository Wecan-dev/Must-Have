...
class Module {
    ...
    public function handle_step_submission() {
        ...
        if ( ! empty( $post_data['save_step'] ) && ! empty( $import_controller->steps[ $import_controller->step ]['handler'] ) ) {
    ...
    public function do_ajax_product_export() {
        ...
        $step     = absint( $post_data['step'] );
        $exporter = new \WC_Product_CSV_Exporter();

        if ( ! empty( $post_data['columns'] ) ) {
            $exporter->set_column_names( $post_data['columns'] );
        }

        if ( ! empty( $post_data['selected_columns'] ) ) {
            $exporter->set_columns_to_export( $post_data['selected_columns'] );
        }

        if ( ! empty( $post_data['export_meta'] ) ) {
            $exporter->enable_meta_export( true );
        }

        if ( ! empty( $post_data['export_types'] ) ) {
            $exporter->set_product_types_to_export( $post_data['export_types'] );
        }
